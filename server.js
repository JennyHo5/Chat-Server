...
io.on('connection', function(socket) {
  ...
  socket.on('clientName', function(data) {
    ...
    registeredUsers.push(socket.id)
    console.log("registeredUsers: " + registeredUsers)
    registerUserNames.push(data)
    console.log("registerUserNames: " + registerUserNames)
  })

  socket.on('clientSays', function(data, isPrivate) {
    console.log('RECEIVED: ' + data)
    let sender = data.slice(0, data.indexOf(':'))
    let message = data.slice(data.lastIndexOf(':') + 1)
    let recieverNames = data.slice(data.indexOf(':') + 1, data.lastIndexOf(':')).split(',')
    recieverNames.push(sender)

    // Defining a custom filter function
    function myFilter(elm){
      return (elm != null && elm !== "" && elm !== " ");
    }

    // Performing filtration
    recieverNames = recieverNames.filter(myFilter);

    if (recieverNames.length >= 2) { //send private message to the server
      for (let i = 0; i < recieverNames.length; i++) {
        let recieverName = recieverNames[i].trim()
        console.log("recieverName: " + recieverName)
        if (registerUserNames.includes(recieverName)) {
          console.log("this is a private message to " + recieverName)
          let index = registerUserNames.indexOf(recieverName)
          let receiver = registeredUsers[index]
          data = sender + ": " + message
          io.to(receiver).emit('serverSays', data, true)
        }
      }
    } else io.emit('serverSays', data, false) //broadcast to everyone including sender
  })
